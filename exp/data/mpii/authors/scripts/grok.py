#!/usr/bin/env python3
"""
Grok AI processing script for transforming video descriptions.
Processes CSV files containing video descriptions and transforms them using Grok AI.

This script reads CSV files with "Description" columns, constructs prompts,
and uses Grok AI to transform the content into coherent movie descriptions.
...
def get_api_key(config):
    ...
    if not api_key or api_key.strip() == '':
        api_key = getpass.getpass('Enter your Grok API key: ')
...
def configure_logging(config):
    ...
    if enable_logging:
        ...
        log_filename = logs_dir / f"grok_{timestamp}.log"
...
class GrokClient:
    """Handles Grok AI API interactions."""
...
def process_csv_files(config, grok_client, folder_path, output_json_file):
    """
    ...
    for csv_file in csv_files:
        ...
        attempt = 0
        response_text = None
        while attempt < grok_client.max_retries:
            try:
                response_text = grok_client.get_response(prompt)
                break
            except Exception as e:
                attempt += 1
                logger.error(f"Error on attempt {attempt} for file ID '{file_id}': {e}")
                if attempt < grok_client.max_retries:
                    logger.info(f"Retrying (attempt {attempt+1}/{grok_client.max_retries})...")
                    time.sleep(grok_client.retry_sleep)
                else:
                    results[file_id] = f"Error after {grok_client.max_retries} attempts: {e}"
                    with open(output_json_file, "w", encoding='utf-8') as json_file:
                        json.dump(results, json_file, indent=4, ensure_ascii=False)
                    logger.error(f"Failed to process file ID '{file_id}' after {grok_client.max_retries} attempts")
                    continue

        # 5) Store result
        if response_text:
            results[file_id] = response_text
            logger.info(f"[File ID '{file_id}'] Stored Grok AI response in results dictionary.")
...
def main():
    ...
    print("=" * 80)
    print("Grok AI Video Description Processing Script")
    print("=" * 80)
    ...
    grok_client = GrokClient(config, api_key)
    
    logger.info("Starting CSV processing and Grok AI querying...")

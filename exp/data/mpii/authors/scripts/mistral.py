#!/usr/bin/env python3
"""
Mistral AI processing script for transforming video descriptions.
Processes CSV files containing video descriptions and transforms them using Mistral AI.

This script reads CSV files with "Description" columns, constructs prompts,
and uses Mistral AI to transform the content into coherent movie descriptions.
...
def configure_logging(config):
    ...
    if enable_logging:
        ...
        log_filename = logs_dir / f"mistral_{timestamp}.log"
...
class MistralClient:
    ...
    def get_response(self, prompt):
        """
        Call Mistral AI's chat.complete endpoint to get a response.
        """
...
def process_csv_files(config, mistral_client, folder_path, output_json_file):
    """
    Process all CSV files in a folder. For each CSV:
      1) Read the "Description" column in order.
      2) Construct a single prompt.
      3) Call Mistral AI with retries on error.
    ...
    for csv_file in csv_files:
        ...
        # 4) Call Mistral AI with retries
        attempt = 0
        response_text = None
        while attempt < mistral_client.max_retries:
            try:
                response_text = mistral_client.get_response(prompt)
                break
            except Exception as e:
                attempt += 1
                logger.error(f"Error on attempt {attempt} for file ID '{file_id}': {e}")
                if attempt < mistral_client.max_retries:
                    logger.info(f"Retrying (attempt {attempt+1}/{mistral_client.max_retries})...")
                    time.sleep(mistral_client.retry_sleep)
                else:
                    results[file_id] = f"Error after {mistral_client.max_retries} attempts: {e}"
                    with open(output_json_file, "w", encoding='utf-8') as json_file:
                        json.dump(results, json_file, indent=4, ensure_ascii=False)
                    logger.error(f"Failed to process file ID '{file_id}' after {mistral_client.max_retries} attempts")
                    continue

        # 5) Store result
        if response_text:
            results[file_id] = response_text
            logger.info(f"[File ID '{file_id}'] Stored Mistral AI response in results dictionary.")
...
def main():
    ...
    print("=" * 80)
    print("Mistral AI Video Description Processing Script")
    print("=" * 80)
    ...
    mistral_client = MistralClient(config, api_key)
    
    logger.info("Starting CSV processing and Mistral AI querying...")
    logger.info(f"Found {len(input_folders)} folders to process")
    
    # Generate output filename - simple name for all parts combined
    output_filename = "mistral.json"
    ...
    for i, folder_path in enumerate(input_folders, 1):
        ...
        try:
            process_csv_files(config, mistral_client, folder_path, str(output_json_file))

"""
Comprehensive Evaluation Framework for Deletion Experiments with Ablation Studies

This module provides a standardized evaluation framework for deletion experiments,
...
# Override _validate_config for deletion-specific validation
class DeletionConfigLoader(ConfigLoader):
    @staticmethod
    def _validate_config(config: Dict) -> None:
        """Validate required configuration fields for deletion experiments."""
        # Call parent validation first
        ConfigLoader._validate_config(config)
        
        # Deletion-specific validation
        required_sections = ['deletion']
    ...
    def load_config(config_path: str) -> Dict:
        """Load configuration with base merging and deletion-specific validation."""
        # Use parent class method for base config merging
        config = ConfigLoader.load_config(config_path)
        # Add deletion-specific validation
        DeletionConfigLoader._validate_config(config)
        return config
...
class StructuredLogger:
    ...
    def setup_loggers(self):
        ...
        main_handler = logging.FileHandler(self.log_dir / f"mpii_deletion_{self.experiment_id}.log")
...
class MetricsEvaluator:
    ...
    def evaluate_single_iteration(
        ...
        try:
                ...
                for Rn in Rn_values:
                    try:
                        ...
                        if vcs_results is None:
                            vcs_results = current_vcs_results
            ...
            # Fallback if no computation succeeded
            if vcs_results is None:
                vcs_results = {
                    "GAS": 0.0, "LAS": 0.0, "NAS": 0.0, "NAS-D": 0.0, "NAS-L": 0.0,
                    "VCS": 0.0, "GAS-LAS-Scaled": 0.0
                ...
                for chunk_size in chunk_sizes:
                    for Rn in Rn_values:
                        ...
                        if current_vcs_results is None:
                            ...
                            current_vcs_results = {
                                "GAS": 0.0, "LAS": 0.0, "NAS": 0.0, "VCS": 0.0,
            ...
            if self.config['deletion'].get('comparison', False):
                # Create comparison metrics with dynamic VCS naming
                comparison_metrics = {
            ...
            if self.logger:
                ...
                self.logger.log_iteration(iteration_id, file_name, subfolder, processing_time, all_metrics)
    ...
    def process_single_file(self, json_file: str, output_folder: str, subfolder: str) -> List[IterationResult]:
        ...
        try:
            ...
            num_iterations = self.config['deletion']['num_iterations']
            
            if self.logger:
                self.logger.log_file_start(json_file, subfolder, num_iterations)
...
class ResultsManager:
        ...
        for file_results in all_results:
            for result in file_results:
                ...
                if result_type == 'ablation':
                    # Use all available ablation metrics (they already have dynamic names)
                    metrics = result.ablation_metrics or {}
                elif result_type == 'comparison':
                    # Use all available comparison metrics (they already have dynamic VCS names)
                    metrics = result.comparison_metrics or {}
                else:
                    continue
                
                if metrics:  # Only add if we have relevant metrics
                    subfolder_results[subfolder].append((result.iteration_id, metrics))
    ...
    def save_aggregated_results(
        ...
        for subfolder, aggregated in aggregated_by_subfolder.items():
            ...
            if result_type == 'ablation':
                csv_path = output_path / f"aggr_ab_del_{subfolder}.csv"
            else:  # comparison
                csv_path = output_path / f"aggr_comp_del_{subfolder}.csv"
            
            df.to_csv(csv_path, index=False)


class EvaluationPipeline:
    """Main evaluation pipeline for deletion experiments."""
    ...
    def __init__(self, config: Dict):
        ...
        # Create deletion-specific logs directory
        deletion_logs_dir = Path(logs_dir) / 'deletion'
        ...
        os.makedirs(deletion_logs_dir, exist_ok=True)
        ...
        self.logger = StructuredLogger(str(deletion_logs_dir), self.experiment_id)
    ...
    def _load_iteration_results_from_checkpoint(self) -> List[List[IterationResult]]:
        """Load IterationResult objects from individual results files for deletion script."""
        import json
        from pathlib import Path
        ...
        try:
            # Process each subfolder (deletion has same structure as addition)
            subfolders = self.config['deletion']['subfolders']
                ...
                for csv_file in sorted(subfolder_dir.glob("*.csv")):
                    ...
                    try:
                        ...
                        for _, row in df_comp.iterrows():
                            ...
                            # Build separate metrics dictionaries
                            comparison_metrics = {}
                            ablation_metrics = {}
                            ...
                            # Add ablation metrics if available
    ...
    def run_evaluation(self) -> None:
        """Execute the complete evaluation pipeline for deletion experiments."""
        
        self.logger.main_logger.info(f"Starting deletion evaluation pipeline")
        ...
        try:
            ...
            if all_results:
                ...
                self.logger.main_logger.info(f"Deletion evaluation completed successfully!")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="VCS Deletion Evaluation Framework - Ablation studies for deletion experiments",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python mpii_eval_deletion.py --config config.yaml
  python mpii_eval_deletion.py --config config.yaml --verbose
...
def main():
    ...
    try:
        # Load configuration
        config = DeletionConfigLoader.load_config(args.config)

...
def generate_summary_text(summary_data: Dict[str, float], segments: List[Dict]) -> str:
    summary_text = (
        ...
        f"• Local NAS Value: {summary_data.get('value', 0):.4f}\n"
...
def generate_calculation_method_text(summary_data: Dict[str, float]) -> str:
    ...
    if actual_length <= ceil_length and actual_length >= floor_length:
        return f"• Calculation: Actual length is within ideal band → Local NAS = 1.0\n"
    elif actual_length < floor_length:
        return (f"• Calculation: Actual < Floor → Local NAS = {actual_length:.2f}"
                f" / {floor_length:.2f}"
                f" = {nas_value:.4f}\n")
    else:
        return (f"• Calculation: Actual > Ceiling → Local NAS = {ceil_length:.2f}"
                f" / {actual_length:.2f}"
                f" = {nas_value:.4f}\n")

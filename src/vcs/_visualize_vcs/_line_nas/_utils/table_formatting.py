...
def create_segment_table_header() -> str:
    ...
    header_text += "│ {:^5} │ {:^20} │ {:^20} │ {:^7} │ {:^7} │ {:^10} │ {:^11} │ {:^9} │ {:^15} │ {:^8} │\n".format(
        "Seg", "Start (x,y)", "End (x,y)", "dx", "dy", "Threshold", "Rn Thresh", "Calculable", "Method", "Length")
    header_text += "├" + "─" * 132 + "┤\n"
    return header_text
...
def format_segment_row(segment: Dict, index: int, Rn: int) -> str:
    ...
    threshold_with_Rn = segment.get('threshold_with_Rn', 'N/A')
    ...
    Rn_thresh_str = f"{threshold_with_Rn:.2f}" if threshold_with_Rn != 'N/A' and Rn > 0 else "N/A"
...
def create_segment_table(segments: List[Dict], Rn: int) -> str:
    ...
    for i, segment in enumerate(segments):
        table_text += format_segment_row(segment, i, Rn)

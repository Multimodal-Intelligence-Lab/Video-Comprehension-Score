...
def draw_mapping_windows_with_Rn(ax: plt.Axes, windows: List[Tuple[int, int]], 
                                 indices: List[int], Rn: int,
...
def should_draw_distance_line(idx: int, target_idx: int, start: int, end: int, 
                             Rn: int, nas_data: Dict[str, Any]) -> bool:
    ...
    return not in_original_window and not in_Rn_window
...
def create_Rn_legend_elements() -> List[Line2D]:

...
def draw_recall_mapping_content(ax: plt.Axes, internals: Dict[str, Any],
    ...
    draw_mapping_windows_with_Rn(
        ax, recall_windows, recall_indices, Rn, 
        gen_len, 'recall'
    )
    ...
    for r_idx, g_idx in enumerate(recall_indices):
        if g_idx >= 0 and r_idx < len(recall_windows):
            ...
            color = determine_point_color(r_idx, g_idx, start, end, Rn, rec_nas_data)
            ax.plot([r_idx], [g_idx], 'o', color=color, ms=6)
            
            # Draw distance lines if needed
            if should_draw_distance_line(r_idx, g_idx, start, end, Rn, rec_nas_data):
...
def draw_recall_penalty_plot(ax: plt.Axes, internals: Dict[str, Any], Rn: int) -> None:
    ...
    # Draw bars
    draw_penalty_bars(ax, penalties, in_window, in_rn_zone, 'lightsalmon')
    ...
    add_penalty_metrics_text(ax, rec_nas_data, 'Recall', Rn)